      ...
      {(<Tabs defaultValue="principal" className="ctl-sheet-tabs">
          ...
          <TabsContent value="principal" data-page-title="Principal" className="ctl-sheet-page">
            <MainSheet className="changeling-main-body"
              ...
              skills={<><SheetHeading>Perícias</SheetHeading>{Object.entries(SKILLS).map(([category, names]) => <TraitBlock key={category} title={category} names={names} values={effectiveSkills} specialties={specialties} highlightedNames={highlightedSkills} highlightTone="kith"/>)}</>}
              specificPowersTitle="Regalias Favorecidas" specificPowers={<><LineList items={changelingFavoredRegalia(data)}/><SheetHeading>Fragilidades</SheetHeading><FrailtyList values={frailties} onChange={(value) => updateLineData(updateSheet, character, "frailties", value)}/></>}
              merits={<><MeritSheetList character={character} merits={principalMerits} updateSheet={updateSheet} catalog={meritCatalog} courtCatalog={lineReference.courts} entitlementCatalog={lineReference.entitlements}/><SheetHeading>Pedras de Contato</SheetHeading><EditableList values={touchstones} minimum={touchstoneSlots} maximum={touchstoneSlots} placeholder={tr("Escreva uma Pedra de Contato", "Write a Touchstone")} onChange={(value) => updateLineData(updateSheet, character, "touchstones", value)}/></>}
            ...
            {/* The shared layout above owns the first page. */}
            {/*
