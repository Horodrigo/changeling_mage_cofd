  ...
  return <><MeritHomebrewPanel line="CtL" catalog={[...catalogs.get<readonly MeritDefinition[]>("core-merits"), ...catalogs.get<readonly MeritDefinition[]>("changeling-merits")]}/><section className="homebrew-panel">
    ...
    <div className="homebrew-source-list">{sources.map(([sourceId, source]) => {
      ...
      return <details className="panel homebrew-source" key={sourceId}>
        <summary className="homebrew-source-summary"><div><Badge variant="outline">{sourceId.startsWith("homebrew:") ? h("Criado pelo jogador", "Player-created") : h("Incluído", "Included")}</Badge><strong>{source}</strong><span>{sourceItems.length} {h("itens implementados", "implemented items")}</span></div></summary>
