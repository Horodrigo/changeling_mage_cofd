...
export type VampireAdvancementUndo =
  ...
  | { kind: "discipline"; name: string }
  | { kind: "bloodPotency" | "humanity" | "willpower" }
  | { kind: "devotion"; id: string }
  | { kind: "cruac"; id: string; humanityLost: number }
  | { kind: "theban"; id: string }
...
export function refundVampireAdvancement(sheet: CharacterSheet, undo: VampireAdvancementUndo) {
  if (undo.kind === "trait") sheet[undo.group][undo.name] = subtractDots(sheet[undo.group][undo.name], 1, undo.group === "attributes" ? 1 : 0);
    ...
    disciplines[undo.name] = subtractDots(disciplines[undo.name]);
    sheet.line_data.disciplines = disciplines;
  } else if (undo.kind === "bloodPotency") sheet.line_data.blood_potency = subtractDots(sheet.line_data.blood_potency, 1, 1);
  else if (undo.kind === "humanity") sheet.line_data.humanity = subtractDots(sheet.line_data.humanity);
  else if (undo.kind === "willpower") sheet.current_state.willpower_lost_dots = Math.max(0, Number(sheet.current_state.willpower_lost_dots ?? 0) + 1);
    ...
    const sorcery = sheet.line_data.blood_sorcery && typeof sheet.line_data.blood_sorcery === "object" && !Array.isArray(sheet.line_data.blood_sorcery)
      ...
      : {};
    if (undo.kind === "cruac") {
      sorcery.cruac_rating = subtractDots(sorcery.cruac_rating);
      sorcery.cruac_rite_ids = without(sorcery.cruac_rite_ids, undo.id);
      ...
      sorcery.theban_rating = subtractDots(sorcery.theban_rating);
      sorcery.theban_miracle_ids = without(sorcery.theban_miracle_ids, undo.id);
    ...
    if (undo.kind === "coil") {
      ...
      ratings[undo.id] = subtractDots(ratings[undo.id]);
