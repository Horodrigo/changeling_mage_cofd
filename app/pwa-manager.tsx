...
export function PwaManager() {
  ...
  useEffect(() => {
    ...
    let checkVersion: (() => Promise<void>) | undefined;
    ...
    if (
      ...
      // A production service worker must never cache Vite's mutable
      // development modules. Remove old registrations and caches before
      // the development client loads them.
        ...
        .then((registration) => {
          checkVersion = async () => {
            ...
            try {
              ...
              // Failure to check the remote version should not interrupt
  ...
  if (
    dismissed ||
    (!installPrompt && !waiting && !newerVersion && online)
  ) {
    return null;
  }
  ...
  return (
    <aside className="pwa-notice" role="status">
      ...
      <span>
        <strong>
          {waiting || newerVersion
            ? tr("Atualização disponível", "Update available")
            : installPrompt
              ? tr("Instale o aplicativo", "Install the app")
              : tr("Você está offline", "You are offline")}
        </strong>
        ...
        <small>
          {waiting
            ...
            : newerVersion
              ? tr(
                  `Preparando a versão ${remoteVersion}…`,
                  `Preparing version ${remoteVersion}…`,
                )
              : installPrompt
                ? tr(
                    "Use Characters of the Darkness pela tela inicial.",
                    "Use Characters of the Darkness from your home screen.",
                  )
                : tr(
                    "Suas fichas locais continuam disponíveis.",
                    "Your local character sheets remain available.",
                  )}
        </small>
      </span>
