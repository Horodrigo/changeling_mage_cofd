...
export type ContractSelection = Pick<ContractDefinition, "id" | "name" | "originalName" | "type" | "regalia" | "description" | "sourceId" | "source" | "page">;
...
function ContractSelector({ contracts, setContracts }: { contracts: ContractSelection[]; setContracts: (value: ContractSelection[]) => void }) {
  ...
  const groups = [...new Set(contractGroups)].map((regalia) => ({ regalia, items: CONTRACTS.filter((item) => item.regalia === regalia && (!normalizedSearch || `${item.name} ${item.originalName} ${item.source} ${item.description}`.toLocaleLowerCase("pt-BR").includes(normalizedSearch))) })).filter((group) => group.items.length);
...
function emptyContract(type: "Comum" | "Real"): ContractSelection { return { id: "", name: "", originalName: "", type, regalia: "", description: "", sourceId: "", source: "", page: 0 }; }
