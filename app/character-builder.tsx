...
import { ArrowLeft, ArrowRight, Check, Minus, Plus, Save, ShieldCheck } from "lucide-react";
...
export function CharacterBuilder({ player, initial, onCancel, onSave }: {
  ...
  const [specialties, setSpecialties] = useState<string[]>(initial?.specializations ?? ["", "", ""]);
  const [aspirations, setAspirations] = useState<string[]>(readArray(initial, "aspirations", ["", "", ""]));
  const [meritText, setMeritText] = useState((initial?.merits ?? []).map((item) => `${item.name} | ${item.dots}`).join("\n"));
  ...
  const merits = useMemo(() => parseMerits(meritText), [meritText]);
  ...
  function validate(nextStep: number) {
    ...
    if (step === 2) {
      ...
      if (specialties.some((item) => !item.trim())) return fail("Preencha as três Especializações.");
    }
    if (step === 3) {
  ...
  function finish() {
    ...
    onSave({
      ...
      skills,
      specializations: specialties.map((item) => item.trim()),
      merits,
      line_data: lineData,
      derived,
      current_state: {},
  ...
  return <section className="builder">
    ...
    <div className="builder-body">
      ...
      {step === 3 && (line === "CtL" ? <CtlStep {...{ seeming, setSeeming, kith, setKith, court, setCourt, needle, setNeedle, thread, setThread, touchstone, setTouchstone, wyrd, setWyrd, secondRegalia, setSecondRegalia, favoredAttribute, setFavoredAttribute, contracts, setContracts, aspirations, setAspirations, meritText, setMeritText, meritBudget, meritSpent }} /> : <MtaStep {...{ path, setPath, order, setOrder, virtue, setVirtue, vice, setVice, nimbus, setNimbus, tool, setTool, resistanceBonus, setResistanceBonus, gnosis, setGnosis, arcana, setArcana, rotes, setRotes, praxes, setPraxes, aspirations, setAspirations, meritText, setMeritText, meritBudget, meritSpent }} />)}
...
function TraitsStep(props: any) {
  return <div className="builder-section"><span className="kicker">PASSO 2</span><h2>Atributos e Perícias</h2><p>Defina a prioridade das categorias e distribua exatamente os pontos indicados.</p><PriorityRow labels={attributeCategories} values={props.attributePriority} setValues={props.setAttributePriority} budgets={[5,4,3]} /><DotGroups groups={ATTRIBUTES} values={props.attributes} setValues={props.setAttributes} base={1} max={5} priority={props.attributePriority} budgets={[5,4,3]} /><div className="section-divider" /><PriorityRow labels={skillCategories} values={props.skillPriority} setValues={props.setSkillPriority} budgets={[11,7,4]} /><DotGroups groups={SKILLS} values={props.skills} setValues={props.setSkills} base={0} max={5} priority={props.skillPriority} budgets={[11,7,4]} /><div className="section-divider" /><h3>Especializações</h3><div className="three-inputs">{props.specialties.map((value: string, index: number) => <Input key={index} value={value} onChange={(e) => updateArray(props.setSpecialties, props.specialties, index, e.target.value)} placeholder={`Especialização ${index + 1}`} />)}</div></div>;
}
...
function Merits({ value, setValue, spent, budget }: any) { return <><div className="merit-heading"><h3>Méritos</h3><Badge variant={spent<=budget ? "secondary" : "destructive"}>{spent}/{budget} pontos</Badge></div><Textarea value={value} onChange={(e)=>setValue(e.target.value)} placeholder={"Um por linha: Nome | pontos\nEx.: Aliado | 2"} /></>; }
...
function parseMerits(value:string) { return value.split("\n").map((line)=>line.trim()).filter(Boolean).map((line)=>{const [name,dots]=line.split("|"); return {name:name.trim(),dots:Math.max(1,Math.min(5,Number(dots)||1))};}); }
