...
export function AnimalCard({ animal, name, onRemove, removable = true, onNameChange }: {
  ...
  return <article className="companion-card">
    <header><div><strong>{name || animal.name}</strong><small>{name ? animal.name : tr("Companheiro animal", "Animal companion")}</small></div>{removable && <Button type="button" size="icon" variant="ghost" onClick={onRemove} aria-label={tr(`Remover ${name || animal.name}`, `Remove ${name || animal.name}`)}><X /></Button>}</header>
    ...
    <p><b>{tr("Atributos", "Attributes")}:</b> {animal.attributes}</p>
    <p><b>{tr("Perícias", "Skills")}:</b> {animal.skills}</p>
    ...
    <p><b>{tr("Deslocamento", "Speed")}:</b> {animal.speed}</p>
    <p><b>{tr("Ataques", "Attacks")}:</b> {animal.attacks.length ? animal.attacks.map(item => `${item.name} ${item.damage} (${item.pool} ${tr("dados", "dice")})${item.note ? ` — ${item.note}` : ""}`).join("; ") : tr("Nenhum", "None")}</p>
    {animal.special && <p><b>{tr("Especial", "Special")}:</b> {animal.special}</p>}
  </article>;
}
