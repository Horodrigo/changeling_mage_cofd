...
import {
  ...
  HardDrive, History, LayoutDashboard, Menu, Minus, Pencil, Plus, Search, ShieldCheck, Sparkles,
...
type ExperienceEntry={id:string;kind:"beat"|"gain"|"spend"|"loss";description:string;beats?:number;experience?:number;createdAt:string};
const BEAT_REASONS=["Aspiração cumprida","Condição resolvida","Ação que concede Beat por uma Condição","Falha dramática voluntária","Rendição em combate","Dano nas caixas mais à direita de Vitalidade","Fim da sessão","Dano de Lucidez","Desvario involuntário","Outro motivo aprovado pelo Narrador"];
...
function ExperiencePanel({character,updateSheet}:{character:CharacterSheet;updateSheet:(sheet:CharacterSheet)=>void}) {
  ...
  const beats=boundedNumber(state.experience_beats,4,0);
  const total=Math.max(0,Math.trunc(Number(state.experience_total??0)||0));
  const spentXp=Math.max(0,Math.trunc(Number(state.experience_spent??0)||0));
  const available=Math.max(0,total-spentXp);
  const history=Array.isArray(state.experience_history)?state.experience_history as ExperienceEntry[]:[];
  const [beatReason,setBeatReason]=useState(BEAT_REASONS[0]);
  ...
  function registerBeat() {
    const next=structuredClone(character); const nextState={...next.current_state}; const converted=beats===4;
    nextState.experience_beats=converted?0:beats+1;
    nextState.experience_total=total+(converted?1:0);
    append({id:crypto.randomUUID(),kind:converted?"gain":"beat",description:converted?`${beatReason} · cinco Beats convertidos em 1 Experiência`:beatReason,beats:1,experience:converted?1:0,createdAt:new Date().toISOString()},nextState);
    next.current_state=nextState; updateSheet(next); setFeedback(converted?"Cinco Beats foram convertidos automaticamente em 1 Experiência.":"Beat registrado.");
  }
  function removeBeat() { if(!beats)return; const next=structuredClone(character); const nextState={...next.current_state,experience_beats:beats-1}; append({id:crypto.randomUUID(),kind:"beat",description:"Correção manual de Beat",beats:-1,createdAt:new Date().toISOString()},nextState); next.current_state=nextState;updateSheet(next);setFeedback("O Beat mais recente foi corrigido."); }
  function markWillpowerLoss() { const maximum=Math.max(0,Number(character.derived.ForçaDeVontade??1)-1);if(lostWillpower>=maximum)return;const next=structuredClone(character);const nextState={...next.current_state,willpower_lost_dots:lostWillpower+1};append({id:crypto.randomUUID(),kind:"loss",description:"Perda permanente de um ponto de Força de Vontade",createdAt:new Date().toISOString()},nextState);next.current_state=nextState;updateSheet(next);setFeedback("Perda permanente de Força de Vontade registrada."); }
  function spend(cost:number,description:string,apply:(next:CharacterSheet)=>void) { if(cost<1||available<cost){setFeedback("Experiência disponível insuficiente para esta compra.");return;}const next=structuredClone(character);apply(next);const nextState={...next.current_state,experience_spent:spentXp+cost};append({id:crypto.randomUUID(),kind:"spend",description,experience:-cost,createdAt:new Date().toISOString()},nextState);next.current_state=nextState;updateSheet(next);setFeedback(`${description} adquirido por ${cost} Experiência${cost===1?"":"s"}.`); }
  function buy() {
    if(purchaseType==="Atributo") { const current=Number(character.attributes[attribute]??1); if(current>=traitMaximum)return setFeedback("Este Atributo já atingiu o máximo permitido pelo Fado."); const target=current+1; spend(4,`${attribute} ${target}`,next=>{next.attributes[attribute]=target;recalculateCtlDerived(next);}); return; }
    if(purchaseType==="Perícia") { const current=Number(character.skills[skill]??0); if(current>=traitMaximum)return setFeedback("Esta Perícia já atingiu o máximo permitido pelo Fado."); const target=current+1; spend(2,`${skill} ${target}`,next=>{next.skills[skill]=target;recalculateCtlDerived(next);}); return; }
    if(purchaseType==="Mérito") { if(!selectedMerit||!nextMeritRating)return setFeedback("Este Mérito não possui outro nível disponível."); const current=ownedMerit?.dots??0; const cost=nextMeritRating-current; spend(cost,`${selectedMerit.translatedName} ${nextMeritRating}`,next=>{const found=next.merits.find((item)=>item.name===selectedMerit.name);if(found)found.dots=nextMeritRating;else next.merits.push({name:selectedMerit.name,dots:nextMeritRating,sourceId:selectedMerit.sourceId,source:selectedMerit.source});}); return; }
    if(purchaseType==="Especialização") { if(!specialtyName.trim())return setFeedback("Informe o nome da Especialização."); spend(1,`Especialização ${specialtySkill}: ${specialtyName.trim()}`,next=>next.specializations.push({skill:specialtySkill,name:specialtyName.trim()})); setSpecialtyName(""); return; }
    if(purchaseType==="Contrato") { if(!selectedContract)return setFeedback("Não há Contrato disponível para esta compra."); const cost=contractExperienceCost(selectedContract,character); spend(cost,`Contrato ${selectedContract.name}`,next=>{const learned=objectList(next.line_data.learned_contracts);next.line_data={...next.line_data,learned_contracts:[...learned,{...selectedContract}]};}); return; }
    if(purchaseType==="Benefício de Contrato") { const value=benefitKey||benefitOptions[0]?.value;if(!value)return setFeedback("Não há Benefício de outra Feição disponível.");const [chosenContract,seeming]=value.split("::");const definition=findContract(chosenContract);spend(1,`Benefício de ${CTL_SEEMING_LABELS[seeming]??seeming} · ${definition?.name??"Contrato"}`,next=>{const existing=objectList(next.line_data.extra_contract_benefits);next.line_data={...next.line_data,extra_contract_benefits:[...existing,{contractId:chosenContract,seeming}]};});return; }
    if(purchaseType==="Fado") { if(wyrd>=10)return setFeedback("Fado já atingiu 10."); spend(5,`Fado ${wyrd+1}`,next=>{next.line_data={...next.line_data,wyrd:wyrd+1};});return; }
    if(!lostWillpower)return setFeedback("O personagem não possui pontos permanentes de Força de Vontade perdidos."); spend(1,"Recuperação de um ponto perdido de Força de Vontade",next=>{next.current_state={...next.current_state,willpower_lost_dots:lostWillpower-1};});
  }
...
function ExperienceRules() { return <div><p><strong>Beats:</strong> cumpra uma Aspiração; resolva uma Condição; aceite uma falha dramática; renda-se em combate; sofra dano nas caixas finais de Vitalidade; encerre uma sessão; sofra dano de Lucidez; ou libere Desvario involuntariamente.</p><p><strong>Custos:</strong> Atributo 4 por ponto; Perícia 2 por ponto; Mérito 1 por ponto; Especialização 1; Contrato Comum 3, Real 4, favorecido Comum 2, favorecido Real 3, Goblin 2; benefício de outra Feição 1; Fado 5 por ponto; ponto perdido de Força de Vontade 1.</p></div>; }
