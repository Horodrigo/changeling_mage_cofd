...
function CharacterPaper({ character, updateState }: { character: CharacterSheet; updateState:(state:Record<string,unknown>)=>void }) {
  ...
  const clarityMaximum=Math.max(1,Number(character.derived.ClarezaMaxima??character.derived.LucidezMaxima??1));
  ...
  return <article className={`cod-sheet ${isCtl?"ctl-sheet":"mta-sheet"}`}>
    ...
    <div className="official-sheet-body">
      ...
      <div className="sheet-right-column">
        ...
        <SheetHeading>Pedras de Toque</SheetHeading><LineList items={[String(data.touchstone??"")]}/>
    ...
    </TabsContent><TabsContent value="poderes" className="ctl-sheet-page powers-page"><SheetHeading>Contratos</SheetHeading><ContractPowerList contracts={contracts} seeming={String(data.seeming??"")}/><div className="powers-sheet-grid"><section><SheetHeading>Outras Características</SheetHeading><LabeledBlank title="Bênção da Feição" lines={3}/><LabeledBlank title="Maldição da Feição" lines={3}/><LabeledBlank title="Bênção da Fratria" lines={3}/><GoblinDebtTrack value={goblinDebt} onChange={(value)=>setState("goblin_debt",value)}/></section><section><SheetHeading>Juramentos</SheetHeading><div className="blank-lines">{Array.from({length:8},(_,index)=><i key={index}/>)}</div><SheetHeading>Méritos Expandidos</SheetHeading><MeritSheetList merits={character.merits} line="CtL"/></section></div><div className="combat-summary"><SheetHeading>Combate</SheetHeading><CompactValues values={{Vitalidade:health,"Força de Vontade":willpower,Defesa:Number(character.derived.Defesa??0),Iniciativa:Number(character.derived.Iniciativa??0),Deslocamento:Number(character.derived.Deslocamento??0),Tamanho:Number(character.derived.Tamanho??5)}}/></div></TabsContent></Tabs>:
...
function ContractPowerList({contracts,seeming}:{contracts:Array<Record<string,unknown>>;seeming:string}) { return <div className="contract-power-list">{contracts.filter((item)=>item.name).map((item,index)=>{const definition=findContract(String(item.id??item.name??""));if(!definition)return null;const benefit=definition.seemingBenefits?.[seeming as keyof typeof definition.seemingBenefits];return <article key={`${definition.id}-${index}`}><div className="contract-power-title"><strong>{definition.name}</strong><Badge variant={definition.goblin?"default":"outline"}>{definition.goblin?"Goblin · Comum":definition.type}</Badge></div><small>{definition.regalia} · {definition.source} · p. {definition.page}</small><p>{definition.description}</p><dl><div><dt>Custo</dt><dd>{definition.cost??"Conforme descrição"}</dd></div><div><dt>Parada de dados</dt><dd>{definition.dicePool}</dd></div><div><dt>Ação / Duração</dt><dd>{definition.action??"Conforme descrição"} · {definition.duration??"Conforme descrição"}</dd></div><div><dt>Brecha</dt><dd>{definition.loophole}</dd></div>{benefit&&<div><dt>Benefício de {CTL_SEEMING_LABELS[seeming]??seeming}</dt><dd>{benefit}</dd></div>}{definition.goblin&&<div className="goblin-debt-row"><dt>Débito Goblin</dt><dd>{definition.goblinDebt}</dd></div>}</dl></article>;})}</div>; }
