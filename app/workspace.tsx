...
function CharacterPaper({ character, updateState, updateSheet }: { character: CharacterSheet; updateState:(state:Record<string,unknown>)=>void; updateSheet:(sheet:CharacterSheet)=>void }) {
  ...
  return <article className={`cod-sheet ${isCtl?"ctl-sheet":"mta-sheet"}`}>
    ...
    <Tabs defaultValue="principal" className="ctl-sheet-tabs mta-sheet-tabs"><TabsList className="ctl-sheet-tab-list" aria-label="Páginas da ficha de Mago"><TabsTrigger value="principal">Principal</TabsTrigger><TabsTrigger value="magia">Magia</TabsTrigger></TabsList><TabsContent value="principal" className="ctl-sheet-page"><section className="sheet-identity-grid"><SheetField label="Nome" value={character.character.name}/><SheetField label="Caminho" value={data.path}/><SheetField label="Ordem" value={MTA_ORDER_LABELS[String(data.order)]??data.order}/><SheetField label="Jogador" value={character.character.player}/><SheetField label="Virtude" value={data.virtue}/><SheetField label="Vício" value={data.vice}/><SheetField label="Crônica" value=""/><SheetField label="Conceito" value={character.character.concept}/><SheetField label="Legado" value={data.legacy}/></section><SheetHeading>Atributos</SheetHeading><div className="official-trait-grid">{Object.entries(ATTRIBUTES).map(([category,names])=><TraitBlock key={category} title={category} names={names} values={character.attributes}/>)}</div><div className="official-sheet-body"><div className="sheet-skills-column"><SheetHeading>Perícias</SheetHeading>{Object.entries(SKILLS).map(([category,names])=><TraitBlock key={category} title={category} names={names} values={character.skills} specialties={specialties}/>)}</div><div className="sheet-center-column"><SheetHeading>Méritos</SheetHeading><MeritSheetList merits={character.merits} line="MtA"/><SheetHeading>Aspirações</SheetHeading><LineList items={aspirations}/><SheetHeading>Obsessões</SheetHeading><LineList items={stringList(data.obsessions)}/></div><div className="sheet-right-column"><SheetHeading>Vitalidade</SheetHeading><HealthTrack health={health} damage={damage} onChange={(value)=>setState("health_damage",value)}/><SheetHeading>Força de Vontade</SheetHeading><ResourceTrack label="Força de Vontade" current={currentWillpower} maximum={willpower} onChange={(value)=>setState("willpower_current",value)}/><SheetHeading>Outras Características</SheetHeading><CompactValues values={character.derived}/><MageExperiencePanel character={character} updateSheet={updateSheet}/></div></div><div className="sheet-bottom-grid"><section><SheetHeading>Condições</SheetHeading><ConditionManager selected={selectedConditions} catalog={MAGE_CONDITIONS} onChange={(value)=>setState("conditions",value)}/></section><section><SheetHeading>Anotações</SheetHeading><NotesArea value={notes} onChange={(value)=>setState("notes",value)}/></section></div></TabsContent><TabsContent value="magia" className="ctl-sheet-page powers-page"><section className="sheet-identity-grid"><SheetField label="Nome" value={character.character.name}/><SheetField label="Caminho" value={data.path}/><SheetField label="Ordem" value={MTA_ORDER_LABELS[String(data.order)]??data.order}/><SheetField label="Ferramenta Mágica Dedicada" value={data.dedicated_tool}/><SheetField label="Nimbus" value={data.nimbus}/><SheetField label="Sabedoria" value={data.wisdom}/></section><div className="mage-magic-grid"><section><SheetHeading>Arcanos</SheetHeading><div className="arcana-sheet-list">{Object.entries(arcana).map(([name,value])=><TraitLine key={name} name={name} value={Number(value)}/>)}</div><SheetHeading>Gnose e Mana</SheetHeading><PowerResource name="Gnose" rating={powerRating} resourceName="Mana" current={currentResource} maximum={resource.maximum} perTurn={resource.perTurn} onChange={(value)=>setState(resourceKey,value)}/></section><section><SheetHeading>Rotas e Práxis</SheetHeading><SpellSheetList rotes={rotes} praxes={praxes}/><SheetHeading>Ferramentas Mágicas</SheetHeading><LineList items={[String(data.dedicated_tool??"")]}/></section></div></TabsContent></Tabs>}
  </article>;
}
...
function BeatTrack({label,value,onChange}:{label:string;value:number;onChange:(value:number)=>void}) { return <div className="beat-resource"><span>{label}</span><div className="resource-track" role="group" aria-label={`${label}: ${value} de 5`}>{Array.from({length:5},(_,index)=><button type="button" key={index} className={index<value?"filled":""} onClick={()=>onChange(index<value?index:index+1)} aria-label={`Definir ${label} como ${index<value?index:index+1}`}/>)}</div><strong>{value}/5</strong></div>; }
