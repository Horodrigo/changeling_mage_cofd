...
function CharacterPaper({ character, updateState, updateSheet }: { character: CharacterSheet; updateState:(state:Record<string,unknown>)=>void; updateSheet:(sheet:CharacterSheet)=>void }) {
  ...
  const selectedConditions=stringList(character.current_state?.conditions);
  ...
  return <article className={`cod-sheet ${isCtl?"ctl-sheet":"mta-sheet"}`}>
    ...
    <div className="sheet-bottom-grid"><section><SheetHeading>Condições</SheetHeading><ConditionManager selectedIds={selectedConditions} onChange={(value)=>setState("conditions",value)}/></section><section><SheetHeading>Anotações</SheetHeading><NotesArea value={notes} onChange={(value)=>setState("notes",value)}/></section></div>
...
function ConditionManager({selectedIds,onChange}:{selectedIds:string[];onChange:(value:string[])=>void}) {
  const [search,setSearch]=useState("");
  const [category,setCategory]=useState("Todas");
  const selected=new Set(selectedIds);
  const filtered=CHANGELING_CONDITIONS.filter((condition)=>(category==="Todas"||condition.category===category)&&`${condition.name} ${condition.originalName} ${condition.description} ${condition.sourceCode}`.toLocaleLowerCase("pt-BR").includes(search.toLocaleLowerCase("pt-BR")));
  return <div className="condition-manager"><div className="selected-conditions">{selectedIds.map(findChangelingCondition).filter(Boolean).map((condition)=>{if(!condition)return null;const inlinePenalty=condition.penalty&&condition.penalty.length<=82;return <div key={condition.id} className="selected-condition" title={`${condition.description}${condition.penalty?`\nEfeito: ${condition.penalty}`:""}\n${condition.source} · p. ${condition.page}`}><span><strong>{condition.name}{condition.persistent?" •":""}</strong>{inlinePenalty&&<>. {condition.penalty}</>}<small>{condition.sourceCode} · p. {condition.page}{condition.penalty&&!inlinePenalty?" · passe o mouse para ver o efeito":""}</small></span><Button type="button" size="icon" variant="ghost" onClick={()=>onChange(selectedIds.filter((id)=>id!==condition.id))} aria-label={`Remover ${condition.name}`}><X/></Button></div>;})}{!selectedIds.length&&<em>Nenhuma Condition selecionada.</em>}</div><Dialog><DialogTrigger asChild><Button type="button" size="sm" variant="outline"><Plus/> Selecionar Condition</Button></DialogTrigger><DialogContent className="condition-dialog"><DialogHeader><DialogTitle>Selecionar Conditions</DialogTitle><DialogDescription>Conditions de Chronicles of Darkness, Changeling the Lost, Hurt Locker e Kith and Kin. O ponto indica uma Condition persistente.</DialogDescription></DialogHeader><div className="condition-filters"><label><Search/><Input value={search} onChange={(event)=>setSearch(event.target.value)} placeholder="Buscar por nome, efeito ou fonte"/></label><RuleSelect value={category} onChange={setCategory} options={["Todas","Mental","Física","Social","Sobrenatural","Changeling"].map((value)=>({value,label:value}))}/></div><div className="condition-catalog">{filtered.map((condition)=><article key={condition.id} className={selected.has(condition.id)?"selected":""}><div><strong>{condition.name}{condition.persistent?" •":""}</strong><small>{condition.originalName} · {condition.sourceCode} · p. {condition.page}</small></div><p>{condition.description}</p>{condition.penalty&&<p className="condition-penalty"><b>Efeito:</b> {condition.penalty}</p>}<Button type="button" size="sm" variant={selected.has(condition.id)?"ghost":"outline"} disabled={selected.has(condition.id)} onClick={()=>onChange([...selectedIds,condition.id])}>{selected.has(condition.id)?"Selecionada":"Adicionar"}</Button></article>)}{!filtered.length&&<em>Nenhuma Condition encontrada.</em>}</div><DialogFooter><DialogClose asChild><Button type="button">Concluir</Button></DialogClose></DialogFooter></DialogContent></Dialog></div>;
}
