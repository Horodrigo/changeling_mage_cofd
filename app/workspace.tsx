    ...
    <Tabs defaultValue="principal" className="ctl-sheet-tabs mta-sheet-tabs">
      ...
      <TabsContent value="magia" className="ctl-sheet-page powers-page mage-spell-page">
        ...
        <div className="mage-page-355-grid">
          <section className="mage-page-left"><SheetHeading>Feitiços Ativos</SheetHeading><EditableList values={stringList(character.current_state?.active_spells)} minimum={Math.max(gnosis,4)} placeholder="Feitiço ativo" onChange={(value)=>setState("active_spells",value)}/><SheetHeading>Conquistas</SheetHeading><EditableList values={stringList(data.attainments)} minimum={5} placeholder="Conquista" onChange={(value)=>updateLineData(updateSheet,character,"attainments",value)}/><SheetHeading>Ferramentas Mágicas</SheetHeading><EditableList values={stringList(data.magical_tools).length?stringList(data.magical_tools):[String(data.dedicated_tool??"")]} minimum={3} placeholder="Ferramenta mágica" onChange={(value)=>updateLineData(updateSheet,character,"magical_tools",value)}/><SheetHeading>Práxis</SheetHeading><SpellColumn items={praxes}/></section>
...
type MageXpEntry={id:string;description:string;regular:number;arcane:number;createdAt:string;before:MageXpSnapshot};
...
function BeatTrack({label,value,onChange}:{label:string;value:number;onChange:(value:number)=>void}) { return <div className="beat-resource"><span>{label}</span><div className="resource-track" role="group" aria-label={`${label}: ${value} de 5`}>{Array.from({length:5},(_,index)=><button type="button" key={index} className={index<value?"filled":""} onClick={()=>onChange(index<value?index:index+1)} aria-label={`Definir ${label} como ${index<value?index:index+1}`}/>)}</div></div>; }
