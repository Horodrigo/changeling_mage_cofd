  ...
  return (
    <section className="homebrew-page">
      <div className="homebrew-hero">
        <div><Badge>{h("CONTEÚDO DO JOGADOR", "PLAYER CONTENT")}</Badge><h2>{h("Oficina de Homebrews", "Homebrew Workshop")}</h2><p>{h("Crie regras próprias e use-as imediatamente em fichas e compras de Experiência neste navegador.", "Create your own rules and use them immediately in character sheets and Experience purchases in this browser.")}</p></div>
      ...
      <section className="panel"><div className="panel-heading"><div><h3>{h("Livros homebrew incluídos", "Included homebrew books")}</h3><p>{h("Conteúdo que acompanha a aplicação e pode ser ativado por livro.", "Content bundled with the application that can be enabled per book.")}</p></div></div><div className="homebrew-grid">{BUILTIN_HOMEBREW_SOURCES.map(source=>{const contracts=CONTRACTS.filter(x=>x.sourceId===source.id);const merits=RAW_MERITS.filter(x=>x.sourceId===source.id);return <article className="homebrew-card" key={source.id}><div><Badge variant="outline">{h("Incluído na aplicação", "Included with the application")}</Badge><h3>{source.name}</h3><p>{localizedCount(locale,contracts.length,"Contrato","Contratos","Contract","Contracts")} · {localizedCount(locale,merits.length,"Mérito","Méritos","Merit","Merits")}</p><details><summary>{h("Ver conteúdo", "View content")}</summary>{contracts.length>0&&<p><strong>{h("Contratos:", "Contracts:")}</strong> {contracts.map(x=>locale==="pt-BR"?x.name:x.originalName).join(", ")}</p>}{merits.length>0&&<p><strong>{h("Méritos:", "Merits:")}</strong> {merits.map(x=>locale==="pt-BR"?x.translatedName:x.name).join(", ")}</p>}</details></div><label className="homebrew-toggle"><span>{catalog.disabledIds.includes(source.id)?h("Desativado","Disabled"):h("Ativo","Active")}</span><Switch aria-label={`${source.name}: ${catalog.disabledIds.includes(source.id)?h("desativado","disabled"):h("ativo","active")}`} checked={!catalog.disabledIds.includes(source.id)} onCheckedChange={checked=>toggle(source.id,checked)} /></label></article>})}</div></section>
      ...
      <Dialog open={editor === "merit"} onOpenChange={(open) => !open && setEditor(null)}><DialogContent className="homebrew-dialog"><DialogHeader><DialogTitle>{h("Criar ou editar Mérito", "Create or edit Merit")}</DialogTitle><DialogDescription>{h("Méritos podem descrever individualmente cada nível adquirido.", "Merits may describe each purchased rating individually.")}</DialogDescription></DialogHeader><MeritForm value={merit} onChange={setMerit} categories={meritCategories} /><EditorFooter error={error} save={save} cancel={() => setEditor(null)} /></DialogContent></Dialog>
