...
export default function Homebrews() {
  ...
  return <section className="homebrew-page">
    ...
    {registration?.loadHomebrew ? <CatalogBoundary groups={registration.catalogGroups.homebrew ?? []}><GameLineHomebrew key={registration.id} gameLine={registration.id}/></CatalogBoundary> : <section className="panel empty-state"><Sparkles/><h3>{h("Nenhum Homebrew implementado", "No implemented Homebrew")}</h3><p>{h("Esta linha ainda não possui conteúdo Homebrew gerenciável.", "This line does not yet have manageable Homebrew content.")}</p></section>}
  </section>;
}
