...
export function HomebrewsPage({
  ...
  const save = () => {
    ...
    if (editor === "kith") {
      if (!kith.name.trim() || !kith.skill.trim() || !kith.blessing.trim()) return setError("Informe nome, Perícia e Bênção do Kith.");
  ...
  return (
    <section className="homebrew-page">
      ...
      <Tabs defaultValue="contracts">
        <TabsList><TabsTrigger value="contracts">Contratos ({catalog.contracts.length})</TabsTrigger><TabsTrigger value="spells">Spells ({catalog.spells.length})</TabsTrigger><TabsTrigger value="merits">Méritos ({catalog.merits.length})</TabsTrigger><TabsTrigger value="kiths">Kiths ({catalog.kiths.length})</TabsTrigger><TabsTrigger value="courts">Cortes ({catalog.courts.length})</TabsTrigger><TabsTrigger value="orders">Ordens ({catalog.orders.length})</TabsTrigger></TabsList>
        ...
        <HomebrewTab value="spells" title="Spells personalizados" text="Feitiços disponíveis como Rotes e Práxis." add={() => openNew("spell")}>
        ...
        <HomebrewTab value="kiths" title="Kiths personalizados" text="Kiths migrados de fichas e criados na oficina." add={()=>openNew("kith")}>{catalog.kiths.map(item=>card("kith",item,item.name,item.skill,item.blessing))}</HomebrewTab>
      ...
      <Dialog open={editor === "spell"} onOpenChange={(open) => !open && setEditor(null)}><DialogContent className="homebrew-dialog"><DialogHeader><DialogTitle>Criar ou editar Spell</DialogTitle><DialogDescription>Os requisitos de Arcana controlarão Rotes e Práxis disponíveis.</DialogDescription></DialogHeader><SpellForm value={spell} onChange={setSpell} /><EditorFooter error={error} save={save} cancel={() => setEditor(null)} /></DialogContent></Dialog>
      ...
      <Dialog open={editor === "kith"} onOpenChange={open=>!open&&setEditor(null)}><DialogContent className="homebrew-dialog"><DialogHeader><DialogTitle>Criar ou editar Kith</DialogTitle></DialogHeader><KithForm value={kith} onChange={setKith}/><EditorFooter error={error} save={save} cancel={()=>setEditor(null)}/></DialogContent></Dialog>
