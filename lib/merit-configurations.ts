...
export type MeritConfigDefinition={name:string;fields:MeritConfigField[];grants?:boolean;line?:"CtL"|"MtA"};

const text=(key:string,label:string,placeholder?:string,minDots?:number):MeritConfigField=>({key,label,placeholder,minDots});
const area=(key:string,label:string,placeholder?:string,minDots?:number):MeritConfigField=>({key,label,kind:"textarea",placeholder,minDots});
const list=(key:string,label:string,placeholder?:string,minDots?:number):MeritConfigField=>({key,label,kind:"list",placeholder,minDots});

export const MERIT_CONFIGURATIONS:MeritConfigDefinition[]=[
 {name:"Area of Expertise",fields:[text("specialty","Especialização beneficiada","Perícia: Especialização")]},
 {name:"Allies",fields:[text("subject","Organização, grupo ou indivíduo")]},
 {name:"Alternate Identity",fields:[text("identity","Nome da identidade"),area("details","Descrição e documentação")]},
 {name:"Contacts",fields:[list("spheres","Esferas de informação","Uma esfera por linha")]},
 {name:"Defensive Combat",fields:[text("skill","Perícia escolhida","Briga ou Armas Brancas")]},
 {name:"Fame",fields:[text("field","Área da fama")]},
 {name:"Fighting Finesse",fields:[text("weapon","Arma ou Briga escolhida")]},
 {name:"Hobbyist Clique",fields:[text("interest","Interesse do círculo"),text("skill","Perícia relacionada")]},
 {name:"Interdisciplinary Specialty",fields:[text("specialty","Especialização beneficiada","Perícia: Especialização")]},
 {name:"Investigative Aide",fields:[text("skill","Perícia escolhida")]},
 {name:"Language",fields:[list("languages","Idiomas","Um idioma por linha")]},
 ...
 {name:"Safe Place",fields:[text("name","Nome do refúgio"),text("location","Localização"),area("details","Descrição e proteções")]},
 {name:"Staff",fields:[text("name","Nome da equipe"),list("services","Serviços e especialidades")]},
 {name:"Status",fields:[text("organization","Organização ou comunidade"),text("role","Cargo ou posição")]},
 {name:"Striking Looks",fields:[area("appearance","Aparência marcante")]},
 ...
 {name:"Exoteric Arete",line:"MtA",fields:[text("skill","Perícia escolhida"),text("specialty","Especialização beneficiada")]},
 {name:"Artifact",line:"MtA",fields:[text("name","Nome do Artefato"),text("gnosis","Gnose"),list("arcana","Arcanos"),list("powers","Poderes e efeitos")]},
 {name:"Broad Dedication",line:"MtA",fields:[text("category","Categoria de ferramentas")]},
 {name:"Cabal Theme",line:"MtA",fields:[text("theme","Tema da Cabala"),list("symbols","Símbolos e comportamentos")]},
 {name:"Cognoscente",line:"MtA",fields:[text("mystery","Mistério ou campo esotérico")]},
 {name:"Daimonomikon",line:"MtA",fields:[text("name","Nome do Daimonomikon"),text("legacy","Legado"),list("attainments","Attainments e ensinamentos")]},
 {name:"Destiny",line:"MtA",fields:[area("destiny","Destino"),area("doom","Ruína")]},
 ...
 {name:"Infamous Mentor",line:"MtA",fields:[text("name","Nome do Mentor"),list("capabilities","Capacidades"),area("obligations","Reputação e obrigações")]},
 {name:"Inheritance",line:"MtA",fields:[area("inheritance","Herança recebida"),list("resources","Recursos"),list("responsibilities","Responsabilidades")]},
 {name:"Library, Advanced",line:"MtA",fields:[list("subjects","Campos esotéricos") ]},
 {name:"Masque",line:"MtA",fields:[text("identity","Identidade"),text("virtue","Virtude alternativa"),text("vice","Vício alternativo"),list("specialties","Especializações próprias"),area("nimbus","Assinatura do Nimbus",undefined,3),list("hubris","Atos de Húbris",undefined,4),list("granted_merits","Méritos concedidos","Nome • pontos",5)],grants:true},
 {name:"Prelacy",line:"MtA",fields:[text("exarch","Exarca patrono"),area("yantra","Simbolismo e Yantra",undefined,2),text("attainment","Attainment",undefined,3),area("temple","Templo",undefined,4)],grants:true},
 {name:"Sanctum",line:"MtA",fields:[text("name","Nome do Santuário"),text("location","Localização"),list("facilities","Instalações") ]},
 {name:"Shadow Name",line:"MtA",fields:[text("shadow_name","Nome das Sombras"),list("symbols","Símbolos favoráveis"),list("contradictions","Contradições e vulnerabilidades")]},
 {name:"Techne",line:"MtA",fields:[text("practice","Prática criativa ou técnica"),list("tools","Métodos e ferramentas")]},
 {name:"Hand of Destiny",line:"MtA",fields:[text("cult","Nome do culto"),list("level_benefits","Benefício de cada nível")],grants:true},
];
...
const parsedDots=(value:string)=>{const bullet=(value.match(/[•●]/g)||[]).length;const number=Number(value.match(/\d+/)?.[0]??0);return Math.max(1,bullet||number||1)};
const cleanGrantName=(value:string)=>value.replace(/[•●]+|\b\d+\b/g,"").replace(/[-–—:]+$/g,"").trim();
export function synchronizeMeritGrants<T extends GrantSheet>(sheet:T):T{
  sheet.merits=sheet.merits.filter(item=>!item.grantedBy);sheet.specializations=sheet.specializations.filter(item=>!item.grantedBy);
  ...
  for(const source of sources){const configuration=normalizeMeritConfiguration(source.configuration);const sourceKey=`merit:${source.name}`;
    ...
    if(source.name==="Prelacy"&&source.dots>=1)conditions.push("Comandos Misteriosos");
